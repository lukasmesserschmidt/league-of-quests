...
class GetLevelQuest(Quest):
    ...
    def requirements_met(cls, game_context: GameContext):
        return game_context.get_data().activePlayer.level <= 18 - cls.level_offset

    def _on_start(self, game_context: GameContext):
        self._current_level = game_context.get_data().activePlayer.level
        self._target_level = self._current_level + self.level_offset

        self._format_description(target_level=self._target_level)
